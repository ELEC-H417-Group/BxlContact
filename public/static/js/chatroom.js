    ...
    $(".myfile").fileinput({
        ...
        showUpload: true, //Whether to display the upload button, the one that follows the text box
        showRemove: false, //Show the remove button, the one that follows the text box
        showCaption: false,//Whether to display the title, it is the text box
        showPreview: true, //Whether to display the preview, if not written, the default is true
        dropZoneTitle: "Please drop the image file here by dragging and dropping",
        dropZoneEnabled: false,//Whether to display the drag area, the default is not written as true, but it will occupy a large area
        maxFileSize: 30720,//The unit is kb, if it is 0, the file size is not limited
	...
	$('.sendBtn').on('click',function(){
		...
		if (toUserId == '' && toGroupId == '') {
			alert("Please choose the conversation partner");
			return;
		}
		if(news == ''){
